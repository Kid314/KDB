...
std::optional<std::string_view> SkipList::get(const std::string& request_key) const
{
    ...
    {
        while (help->forward[i]!=nullptr&&help->forward[i]->value<request_key)
        {
            help=help->forward[i];
        }
    }
...
void SkipList::put(std::string& request_key, std::string& request_value)
{
    ...
    {
        for (int i=temp_level;i>current_level;--i)
        {
            mem_vec[i-1]=nullptr;
        }
    ...
    if ()
    Node* new_node=new Node(std::move(request_key),std::move(request_value),num_generate());

}
void SkipList::remove(const std::string&)
{
